from hikari import Embed, Message, GuildMessageCreateEvent
...
class Minesweeper:
    ...
    async def start(self) -> None:
        self.embed = (Embed(color=f'{randint(0, 0xFFFFFF):06x}')
                      .set_author(name='Partie de démineur', icon=self.ctx.author.avatar_url)
    ...
    async def loop(self, init: bool = False) -> None:
        event = await self.bot.wait_for(GuildMessageCreateEvent, timeout=None, predicate=lambda m: m.author == self.ctx.author)
