from hikari import Role, TextableChannel, Embed, Permissions, GuildJoinEvent, GuildLeaveEvent, MemberCreateEvent, MemberDeleteEvent
from lightbulb import Plugin, Context, SlashCommand, command, option, implements, guild_only, has_guild_permissions
...
async def on_guild_join(event):
    ...
    await plugin.bot.db.setup.insert({'_id': guild.id, 'mute': None, 'logs': None, 'channel': None, 'new': None, 'welcome': None})
    for member in filter(lambda m: not m.bot, guild.members):
        await plugin.bot.db.members.update({'_id': member.id}, {'$addToSet': {'guilds': {'id': guild.id, 'level': 0, 'xp':0}}}, True)
...
async def on_guild_remove(event):
    try:
        ...
        await plugin.bot.db.members.collection.update_many({'_id': {'$in': [member.id for member in guild.members]}}, {'$pull': {'guilds': {'id': guild.id}}})
    except:
        return
...
async def on_member_join(event):
    ...
    await plugin.bot.db.members.update({'_id': user.id}, {'$addToSet': {'guilds': {'id': guild.id, 'level': 0, 'xp': 0}}}, True)


@plugin.listener(MemberDeleteEvent)
async def on_member_remove(event):
    user, guild = event.user, event.get_guild()
    await plugin.bot.db.members.update({'_id': user.id}, {'$pull': {'guilds': {'id': guild.id}}})
