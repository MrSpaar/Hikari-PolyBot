...
async def rank(ctx: Context):
    ...
    data = await plugin.bot.db.members.sort({'guilds.id':guild.id}, {'guilds.$': 1}, 'guilds.xp', -1)
...
async def levels(ctx: Context):
    ...
    data = await plugin.bot.db.members.sort({'guilds.id':752921557214429316}, {'guilds.$': 1}, 'guilds.xp', -1)
...
async def on_reaction_add(event):
    ...
    if not message or not message.embeds or not message.embeds[0].author or message.embeds[0].author.name != 'Classement du serveur':
        return
...
async def on_message(event):
    ...
    entry = await plugin.bot.db.members.find({'guilds.id': guild.id, '_id': member.id}, {'guilds.$': 1})
    if not entry:
        return
    ...
    await plugin.bot.db.members.update({'guilds.id': guild.id, '_id': member.id},
