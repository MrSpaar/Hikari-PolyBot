...
class DB:
    ...
    @staticmethod
    async def delete_server(guild: GatewayGuild) -> None:
        await DB.cluster["setup"].delete_one({'_id': guild.id})

        filt = {"_id": {"$in": [member.id for member in guild.get_members()]}}
        query = {"$pull": {"guilds": {"id": guild.id}}}

        await DB.cluster["members"].update_many(filt, query)

    @staticmethod
    async def add_server(guild: GatewayGuild) -> None:
        await DB.cluster["setup"].insert_one({
            "_id": guild.id,
            "logs": None,
            "channel": None,
            "new": None,
        ...
        for member in members:
            await DB.add_member_guild(guild.id, member.id)
